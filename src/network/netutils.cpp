...
void NetUtils::writeMultipleSplitsToSocket(int socket, const std::vector<Split*>& splits) {
    size_t totalSize = 0;
    for (const auto* split : splits) {
        totalSize += 9 + split->content_length;
    }
    ...
    for (const auto* split : splits) {
        ...
        buffer.resize(oldSize + 9);
        
        buffer[oldSize] = INITIAL_WRITE_FLAG;
        
        unsigned char idBytes[4];
        encodeIntToUchar(idBytes, split->id);
        std::copy(idBytes, idBytes + 4, buffer.begin() + oldSize + 1);
        
        unsigned char lengthBytes[4];
        encodeIntToUchar(lengthBytes, static_cast<int>(split->content_length));
        std::copy(lengthBytes, lengthBytes + 4, buffer.begin() + oldSize + 5);
        
        if (split->content_length > 0) {
            buffer.insert(buffer.end(), split->content.begin(), split->content.end());
        }
    }
